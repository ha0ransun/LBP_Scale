...
import torchvision
from PAS.data_util.raw_dataset import load_raw_dataset
from PAS.model.dnn import get_model
from PAS.model.debm import EBM
from PAS.learning.replay_buffer import ReplayBuffer
from PAS.sampling.sampler import get_sampler
from PAS.common.rate_stats import get_rate_stats
from PAS.data_util.data_loader import get_data_loader
import PAS.sampling.ais as ais
...
def main_loop(args, save_fig, eval_metric):
    ...
    if args.eval_only:
        ...
        sampler = get_sampler(args, sampler='gwg', log_g=lambda x: x / 2.0)
    ...
    n_epochs = 0
    best_val_ll = -1e10    
    rs_tracker = get_rate_stats(args.rate_stats)
    while itr < args.n_iters:
        ...
        for x in pbar:
            ...
            for _ in range(args.sampling_steps):
                x_fake_new = sampler.step(x_fake.detach(), model).detach()
                x_fake = x_fake_new
                rs_tracker.step(sampler.step_acc, x_fake.shape[0])
            adaptive_msg = ''
            if args.adaptive_sampling:
                cur_acc = rs_tracker.get()
                adaptive_msg = sampler.adaptive_adjust(cur_acc)
            ...
            log = 'iter: %d, lr: %.2e, obj: %.2f, score_true: %.2f, score_fake: %.2f %s' % (itr, lr, obj, score_real, score_fake, adaptive_msg)
