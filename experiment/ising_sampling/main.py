...
from lbp.model.ising import Ising
...
def main(args):
    ...
    model = Ising(p=args.p, mu=args.mu, sigma=args.sigma, lamda=args.lamda, device=device)
    args.g_func = "sqrt2"
    methods1 = ['rw', 'GWG', 'lbp']
    stats = {}
    ...
    if not os.path.isdir('results'):
        os.mkdir('results')
    with open(f'results/p-{args.p}_mu-{args.mu}_sigma-{args.sigma}_lambda-{args.lamda}.pkl', 'wb') as handle:
