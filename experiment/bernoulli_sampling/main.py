...
from lbp.model.bernoulli import Bernoulli
...
def main(args):
    ...
    model = Bernoulli(p=args.p, alpha=args.alpha, beta=args.beta, device=device)
    args.g_func = "sqrt2"
    methods1 = ['rw', 'GWG', 'lbp']
    stats = {}
    ...
    if not os.path.isdir('results'):
        os.mkdir('results')
    with open(f'results/p-{args.p}_alpha-{args.alpha}_beta-{args.beta}.pkl', 'wb') as handle:
